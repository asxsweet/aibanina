...
// --- Date plan options ("where to go" list, fully admin-managed) -----------
//
// The admin adds/edits/deletes these from the admin panel; the partner
// picks one herself (alongside her own day/time choice) when she responds
// to an invite. Ordered by `order` so the admin's arrangement is stable.

interface IDatePlanOption {
  id: string;
  label: string;
  order: number;
  createdAt: number;
...
const DatePlanOptionSchema = new mongoose.Schema<IDatePlanOption>({
  id: { type: String, required: true, unique: true },
...
const DatePlanOptionModel =
  (mongoose.models.DatePlanOption as mongoose.Model<IDatePlanOption>) ||
  mongoose.model<IDatePlanOption>('DatePlanOption', DatePlanOptionSchema);

// A nice default list so the picker isn't empty the very first time either
// of them opens the app — only ever inserted once, the first time the list
// is fetched and the collection turns out to be empty. The admin is free to
// edit or delete every single one afterwards; this never re-seeds once any
// option (default or custom) exists.
const DEFAULT_DATE_PLAN_OPTIONS = [
  '☕ Кофе', '🎬 Кино', '🍝 Кешкі ас', '🌳 Саябақта серуен', '🍹 Дринк', '🎁 Сюрприз',
];

// GET /api/date-plan-options — the full admin-managed "where to go" list,
// used both by the admin panel (to manage) and the partner's picker.
app.get('/api/date-plan-options', async (req, res) => {
  if (!(await ensureMongo(res))) return;
  try {
    let options = await DatePlanOptionModel.find({}).sort({ order: 1, createdAt: 1 }).lean();
    if (options.length === 0) {
      const seeded = await DatePlanOptionModel.insertMany(
        DEFAULT_DATE_PLAN_OPTIONS.map((label, i) => ({
          id: 'plan_' + Date.now().toString(36) + i,
          label,
          order: i,
        }))
      );
      options = seeded.map((doc) => doc.toObject());
    }
    return res.json({ options });
  } catch (err) {
    console.error('Error fetching date plan options:', err);
    return res.status(500).json({ error: 'Failed to fetch date plan options' });
  }
});

// POST /api/date-plan-options — admin adds a new destination option.
app.post('/api/date-plan-options', async (req, res) => {
  ...
  if (!(await ensureMongo(res))) return;
  try {
    const count = await DatePlanOptionModel.countDocuments({});
    const id = 'plan_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
    const created = await DatePlanOptionModel.create({ id, label: label.trim().slice(0, 60), order: count });
    return res.json({ success: true, option: created });
  } catch (err) {
    console.error('Error creating date plan option:', err);
    return res.status(500).json({ error: 'Failed to create date plan option' });
  }
});

// PUT /api/date-plan-options/:id — admin edits an option's label.
app.put('/api/date-plan-options/:id', async (req, res) => {
  const { id } = req.params;
  const { label } = req.body;
  if (typeof label !== 'string' || !label.trim()) {
    return res.status(400).json({ error: 'label is required' });
  ...
  if (!(await ensureMongo(res))) return;
  try {
    const updated = await DatePlanOptionModel.findOneAndUpdate(
      { id },
      { label: label.trim().slice(0, 60) },
      { new: true }
    ).lean();
    if (!updated) return res.status(404).json({ error: 'Option not found' });
    return res.json({ success: true, option: updated });
  } catch (err) {
    console.error('Error updating date plan option:', err);
    return res.status(500).json({ error: 'Failed to update date plan option' });
  }
});

// DELETE /api/date-plan-options/:id — admin removes a destination option.
app.delete('/api/date-plan-options/:id', async (req, res) => {
  const { id } = req.params;
  if (!(await ensureMongo(res))) return;
  try {
    const result = await DatePlanOptionModel.deleteOne({ id });
    if (result.deletedCount === 0) return res.status(404).json({ error: 'Option not found' });
    return res.json({ success: true, id });
  } catch (err) {
    console.error('Error deleting date plan option:', err);
    return res.status(500).json({ error: 'Failed to delete date plan option' });
  }
});

// --- Date invite ("will you go out with me?" flow) -------------------------
//
// IMPORTANT: the admin only ever manages the destination OPTIONS LIST above
// and an optional personal note when granting access from the admin panel.
// WHICH option, WHICH day and WHAT time are all deliberately left null at
// creation — the partner picks all three herself, right after she says yes,
// exactly like the reference video's flow (ask -> she picks place/day/time
// -> letter). Only the most recently created invite is ever considered
// "active" — sending a new one supersedes the last one without needing an
// explicit cancel/delete step.

interface IDateInvite {
  id: string;
  day: string | null;
  time: string | null;
  plan: string | null;
...
const DateInviteSchema = new mongoose.Schema<IDateInvite>({
  id: { type: String, required: true, unique: true },
...
const DateInviteModel =
  (mongoose.models.DateInvite as mongoose.Model<IDateInvite>) ||
  mongoose.model<IDateInvite>('DateInvite', DateInviteSchema);

// GET /api/date-invite/active — the single most recent invite (any status),
// or null if the admin has never sent one. Older invites are just history;
// nothing ever explicitly deletes them, a newer one simply supersedes.
app.get('/api/date-invite/active', async (req, res) => {
  if (!(await ensureMongo(res))) return;
  try {
    const invite = await DateInviteModel.findOne({}).sort({ createdAt: -1 }).lean();
    return res.json({ invite: invite || null });
  } catch (err) {
    console.error('Error fetching active date invite:', err);
    return res.status(500).json({ error: 'Failed to fetch date invite' });
  }
});

// POST /api/date-invite — admin grants access with just an optional note;
// deliberately no plan/day/time yet — she chooses all three herself in her
// own respond step, from the admin-managed options list above. Doesn't
// require an admin check server-side (consistent with the rest of this
// app's trust model — there's no session system, the admin-only button is
// a frontend-only gate), but only the admin's UI ever calls this.
app.post('/api/date-invite', async (req, res) => {
  const { note } = req.body;
  if (!(await ensureMongo(res))) return;

  try {
    const id = 'invite_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 8);
    const created = await DateInviteModel.create({
      id,
      day: null,
      time: null,
      plan: null,
      planLabel: null,
      note: typeof note === 'string' ? note.trim().slice(0, 400) : '',
      status: 'pending',
      dodgeCount: 0,
      respondedAt: null,
    });
    return res.json({ success: true, invite: created });
  } catch (err) {
    console.error('Error creating date invite:', err);
    return res.status(500).json({ error: 'Failed to create date invite' });
  }
});

// POST /api/date-invite/:id/respond — the partner says yes AND submits the
// place/day/time she picked in the same call (she only calls this once
// she's finished the whole "ask -> pick place/day/time" flow, mirroring the
// video where the letter only appears after both steps). Only ever moves
// 'pending' -> 'accepted'; there's no decline endpoint by design — the "No"
// button only dodges, it never actually submits (see plan doc).
app.post('/api/date-invite/:id/respond', async (req, res) => {
  const { id } = req.params;
  const { day, time, plan, planLabel, dodgeCount } = req.body;
  if (
    ...
    return res.status(400).json({ error: 'day, time, plan and planLabel are required' });
  }
  if (!(await ensureMongo(res))) return;

  try {
    const updated = await DateInviteModel.findOneAndUpdate(
      { id, status: 'pending' },
      {
        day,
        time,
        ...
        dodgeCount: typeof dodgeCount === 'number' && dodgeCount >= 0 ? Math.floor(dodgeCount) : 0,
      },
      { new: true }
    );
    if (!updated) return res.status(404).json({ error: 'Pending invite not found' });
    return res.json({ success: true, invite: updated });
  } catch (err) {
    console.error('Error responding to date invite:', err);
    return res.status(500).json({ error: 'Failed to respond to date invite' });
  }
});

// PUT /api/date-invite/active — admin edits the still-pending invite's note
// (e.g. changed her mind about what to say) without having to retract and
// resend a brand-new one.
app.put('/api/date-invite/active', async (req, res) => {
  const { note } = req.body;
  ...
  try {
    const latest = await DateInviteModel.findOne({}).sort({ createdAt: -1 });
    if (!latest || latest.status !== 'pending') {
      return res.status(404).json({ error: 'No pending invite to edit' });
    }
    latest.note = note.trim().slice(0, 400);
    await latest.save();
    return res.json({ success: true, invite: latest });
  } catch (err) {
    console.error('Error editing date invite:', err);
    return res.status(500).json({ error: 'Failed to edit date invite' });
  }
});

// DELETE /api/date-invite/active — the admin retracts the current invite
// (right button in the dashboard's "Свидание" tab): the section goes back
// to its locked state as if nothing had been sent yet.
app.delete('/api/date-invite/active', async (req, res) => {
  if (!(await ensureMongo(res))) return;
  try {
    const latest = await DateInviteModel.findOne({}).sort({ createdAt: -1 });
    if (!latest) return res.json({ success: true, deleted: false });
    await DateInviteModel.deleteOne({ _id: latest._id });
    return res.json({ success: true, deleted: true });
  } catch (err) {
    console.error('Error deleting date invite:', err);
    return res.status(500).json({ error: 'Failed to delete date invite' });
  }
});
...
// PUT /api/admin/users-progress/:userId — the dashboard's editable
// "Серіктес" tab: lets the admin directly correct streak/daysPlayed/
// lastPlayedDate/unlockedDays for any account (e.g. after manually
// verifying a bug, or catching them up if something desynced).
app.put('/api/admin/users-progress/:userId', async (req, res) => {
  const { userId } = req.params;
  const { daysPlayed, streak, lastPlayedDate, unlockedDays } = req.body;
  if (!(await ensureMongo(res))) return;

  const update: Record<string, unknown> = {};
  if (typeof daysPlayed === 'number' && daysPlayed >= 0) update.daysPlayed = Math.floor(daysPlayed);
  if (typeof streak === 'number' && streak >= 0) update.streak = Math.floor(streak);
  if (lastPlayedDate === null || (typeof lastPlayedDate === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(lastPlayedDate))) {
    update.lastPlayedDate = lastPlayedDate;
  }
  if (Array.isArray(unlockedDays) && unlockedDays.every((d) => typeof d === 'number')) {
  ...
  try {
    const updated = await UserProgressModel.findOneAndUpdate({ userId }, update, { new: true }).lean();
...
// POST /api/admin/users-progress/:userId/reset — full wipe of an account's
// game progress (streak, days played, unlocked days, last-played date) back
// to a brand-new state. Theme stays as-is — resetting progress shouldn't
// take away a theme she already unlocked/chose.
app.post('/api/admin/users-progress/:userId/reset', async (req, res) => {
  const { userId } = req.params;
  if (!(await ensureMongo(res))) return;
  try {
    const updated = await UserProgressModel.findOneAndUpdate(
      { userId },
      { daysPlayed: 0, streak: 0, lastPlayedDate: null, lastPlayTimestamp: 0, unlockedDays: [] },
      { new: true }
    ).lean();
    if (!updated) return res.status(404).json({ error: 'User not found' });
    return res.json({ success: true, user: updated });
...
// --- Telegram reminders ----------------------------------------------------
//
// The daily star resets at local midnight, but nobody's staring at the app
// waiting for that — so instead the server itself pings Telegram: once right
// when the new star becomes available, and again in the evening ONLY if
// that day's star still hasn't been claimed. Both are entirely optional —
// if the bot isn't configured, this quietly does nothing.
//
// Setup (one-time):
//   1. Message @BotFather on Telegram -> /newbot -> copy the token it gives you.
//   2. Have the person who should get reminders send /start to your new bot.
//   3. Open https://api.telegram.org/bot<TOKEN>/getUpdates in a browser —
//      the reply contains her numeric "chat":{"id": ...} — that's TELEGRAM_CHAT_ID.
//   4. Put all four values below into your .env (see .env.example).
const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN || '';
const TELEGRAM_CHAT_ID = process.env.TELEGRAM_CHAT_ID || '';
// Which player's progress the evening check-up watches — her account email
// (the same value used as UserProgress.userId).
const TELEGRAM_WATCH_USER_EMAIL = process.env.TELEGRAM_WATCH_USER_EMAIL || '';
// IANA timezone the reminders are scheduled in (her local time), independent
// of whatever timezone the hosting server itself happens to run in.
...
async function sendTelegramMessage(text: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      console.error('Telegram API қатесі:', res.status, await res.text());
    }
  } catch (err) {
    console.error('Telegram хабарлама жіберу сәтсіз аяқталды:', err);
  }
}

// "Today", computed in APP_TIMEZONE regardless of the server's own OS
// timezone — the same YYYY-MM-DD shape UserProgress.lastPlayedDate uses.
...
function setupTelegramReminders(): void {
  if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID) {
    console.log('ℹ️ TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID орнатылмаған — Telegram еске салғыштар өшірулі.');
    return;
  }

  // 00:05 — the new day's star just became available.
  cron.schedule(
    '5 0 * * *',
    () => {
      sendTelegramMessage('⭐ Жаңа жұлдызың дайын болды! Кел, оны алуға асық.');
    },
    { timezone: APP_TIMEZONE }
  );

  // 20:00 — a gentle nudge, but only if today's star is still unclaimed.
  cron.schedule(
    '0 20 * * *',
    async () => {
      if (!TELEGRAM_WATCH_USER_EMAIL) return;
      if (mongoose.connection.readyState !== 1) return;
      try {
        const progress = await UserProgressModel.findOne({ userId: TELEGRAM_WATCH_USER_EMAIL }).lean();
        const today = getDateStringInTimezone(APP_TIMEZONE);
        if (!progress || progress.lastPlayedDate !== today) {
          sendTelegramMessage('💫 Бүгін әлі жұлдызыңды алған жоқсың — ұмытып кетпе!');
        }
      } catch (err) {
        console.error('Telegram еске салғышты тексеру қатесі:', err);
      }
    },
    { timezone: APP_TIMEZONE }
  );

  console.log(`✅ Telegram еске салғыштар қосылды (${APP_TIMEZONE} уақыты бойынша).`);
}
