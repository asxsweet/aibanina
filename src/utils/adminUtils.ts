...
// Directly corrects a user's streak/daysPlayed/lastPlayedDate/unlockedDays —
// the "Серіктес" tab's inline editor, for fixing a desynced account without
// needing her to replay days.
export async function updateUserProgress(userId: string, update: UserProgressUpdate): Promise<void> {
  const res = await fetch(`${API_BASE}/api/admin/users-progress/${encodeURIComponent(userId)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(update),
  });
  if (!res.ok) throw new Error('Прогресті сақтау мүмкін болмады.');
}

// Full wipe of a user's game progress back to a brand-new state (theme is
// left untouched). Used by the "Серіктес" tab's danger-zone reset button.
export async function resetUserProgress(userId: string): Promise<void> {
  const res = await fetch(`${API_BASE}/api/admin/users-progress/${encodeURIComponent(userId)}/reset`, {
    method: 'POST',
  });
  if (!res.ok) throw new Error('Прогресті нөлдеу мүмкін болмады.');
}
