...
export const MessageCard: React.FC<MessageCardProps> = ({
  ...
  // Reset the "voice failed to load" flag whenever a different day's
  // message is shown, so a broken file on one day doesn't hide the player
  // for the next day that actually has a working voice message.
  useEffect(() => {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--overlay)] backdrop-blur-md">
        ...
        >
          ...
          {/* Voice message player — shown on weekly voice-message days (7, 14, 21, 28...).
              If the file 404s or fails to decode, this quietly swaps to a
              small notice instead of leaving a broken, silent player on
              screen (which is what a missing/renamed mp3 used to show). */}
          {message.voiceUrl && (
            <div className="mb-4 p-3.5 rounded-2xl bg-[var(--bg-card)] border border-[var(--border)] text-left">
              ...
              {voiceUnavailable ? (
                <p className="text-xs text-[var(--text-faint)] italic">
                  Дауыс файлы әзірге қолжетімсіз.
                </p>
              ) : (
                <audio
                  controls
